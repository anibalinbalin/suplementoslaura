...
import { MedicationInteractionService, InteractionSeverity } from './medication-interaction-service'
...
// Instancia global del servicio de interacciones
let interactionService: MedicationInteractionService | null = null

// Inicializar el servicio de interacciones
const initializeInteractionService = async () => {
  if (!interactionService) {
    interactionService = new MedicationInteractionService()
...
export const verificarInteraccionesMedicamentos = (nombreSuplemento: string, medicamentos: string): string[] => {
  ...
  // Usar el servicio de interacciones mejorado si está disponible
  if (interactionService) {
    const interactions = interactionService.checkInteractions(nombreSuplemento, medicamentos)
    
    // Formatear las interacciones según el nivel de severidad
    return interactions.map(interaction => {
      ...
      let message = `${severityIcon}: ${interaction.warning}`
      if (interaction.recommendation) {
        message += ` Recomendación: ${interaction.recommendation}`
      }
  ...
  // Fallback al sistema anterior si el servicio no está disponible
...
initializeInteractionService().catch(error => {
  console.error('Error initializing medication interaction service:', error)
})

// Obtener recomendaciones específicas para un suplemento
export const obtenerRecomendacionesMedicamentos = async (nombreSuplemento: string): Promise<string[]> => {
  const service = await initializeInteractionService()
  return service.getSupplementRecommendations(nombreSuplemento)
}

// Obtener URL de drugs.com para verificación
export const obtenerUrlDrugsCom = async (nombreSuplemento: string): Promise<string | undefined> => {
  const service = await initializeInteractionService()
  return service.getDrugsComUrl(nombreSuplemento)
}
...
// Suplementos adicionales para análisis de sangre
const suplementosAnalisisSangre: DatosSuplemento[] = [
  {
    nombre: "Berberina",
    descripcion: "Compuesto bioactivo extraído de varias plantas, con potentes efectos sobre el metabolismo de la glucosa y la sensibilidad a la insulina.",
    beneficios: [
      "Mejora significativamente la sensibilidad a la insulina",
      "Reduce los niveles de glucosa en sangre de manera comparable a la metformina",
      "Disminuye los triglicéridos y el colesterol LDL",
      "Apoya la pérdida de peso, especialmente grasa abdominal",
      "Tiene propiedades antiinflamatorias",
      "Mejora la salud cardiovascular"
    ],
    dosificacion: {
      general: "500 mg tres veces al día con las comidas principales",
      hombres: "1500 mg diarios divididos en 3 dosis",
      mujeres: "1000-1500 mg diarios divididos en 2-3 dosis"
    },
    momento_optimo: "Con las comidas principales para evitar molestias gastrointestinales",
    consejos_absorcion: "Tomar con alimentos para mejorar tolerancia. Puede interactuar con medicamentos para diabetes.",
    efectos_secundarios: [
      "Molestias gastrointestinales leves al inicio",
      "Puede causar estreñimiento en algunas personas",
      "Interacciones con medicamentos metabolizados por el hígado"
    ],
    evidencia_cientifica: [
      "Múltiples estudios muestran eficacia similar a metformina en diabetes tipo 2",
      "Reduce HbA1c en 0.9% en promedio en diabéticos",
      "Mejora HOMA-IR significativamente en síndrome metabólico"
    ],
    preguntas_frecuentes: [
      {
        pregunta: "¿La berberina es tan efectiva como la metformina?",
        respuesta: "Estudios muestran eficacia comparable en control glucémico, pero siempre consulte con su médico antes de cambiar medicamentos."
      },
      {
        pregunta: "¿Cuánto tiempo tarda en hacer efecto?",
        respuesta: "Los efectos sobre la glucosa se notan en 2-4 semanas, pero los beneficios completos pueden tardar 3 meses."
      }
    ]
  },
  {
    nombre: "Ácido Fólico",
    descripcion: "Forma sintética del folato (vitamina B9), esencial para la síntesis de ADN y la formación de glóbulos rojos.",
    beneficios: [
      "Previene defectos del tubo neural en embarazadas",
    ...
    dosificacion: {
      general: "400-800 mcg diarios",
      hombres: "400 mcg diarios",
      mujeres: "400-800 mcg diarios (800 mcg si planea embarazo)"
    },
    momento_optimo: "Con el desayuno o la primera comida del día",
    consejos_absorcion: "Mejor absorción con vitamina C. Evitar con té negro que inhibe absorción.",
    efectos_secundarios: [
      "Generalmente muy seguro",
      "Puede enmascarar deficiencia de B12 en dosis altas",
      "Raramente causa molestias gastrointestinales"
    ],
    evidencia_cientifica: [
      "Reduce defectos del tubo neural en 70% cuando se toma antes del embarazo",
      "Disminuye homocisteína elevada, factor de riesgo cardiovascular",
      "Puede mejorar la respuesta a antidepresivos"
    ],
    preguntas_frecuentes: [
      {
        pregunta: "¿Es lo mismo ácido fólico que folato?",
        respuesta: "El ácido fólico es la forma sintética, mientras que el folato es la forma natural. Ambos son efectivos."
      },
      {
        pregunta: "¿Cuándo debo tomarlo si planeo embarazarme?",
        respuesta: "Idealmente 3 meses antes de la concepción y durante el primer trimestre."
      }
    ]
  },
  {
    nombre: "L-Teanina",
    descripcion: "Aminoácido único del té verde que promueve la relajación sin causar somnolencia, mejorando el estado de alerta calmado.",
    beneficios: [
      "Reduce el estrés y la ansiedad sin sedación",
      "Mejora la calidad del sueño",
      "Aumenta las ondas alfa cerebrales (estado de relajación)",
      "Potencia los efectos cognitivos de la cafeína",
      "Puede reducir la presión arterial en situaciones de estrés",
      "Mejora el enfoque y la atención"
    ],
    dosificacion: {
      general: "200-400 mg diarios",
      hombres: "200-400 mg según necesidad",
      mujeres: "100-200 mg para empezar, hasta 400 mg"
    },
    momento_optimo: "30-60 minutos antes de situaciones estresantes o antes de dormir",
    consejos_absorcion: "Se puede tomar con o sin alimentos. Combina bien con cafeína para enfoque sin nerviosismo.",
    efectos_secundarios: [
      "Muy bien tolerada",
      "Raramente causa dolor de cabeza leve",
      "Puede causar somnolencia en dosis muy altas"
    ],
    evidencia_cientifica: [
      "Reduce marcadores de estrés en 1 hora después de la ingesta",
      "Mejora la calidad del sueño sin causar somnolencia diurna",
      "Aumenta ondas alfa cerebrales asociadas con relajación alerta"
    ],
    preguntas_frecuentes: [
      {
        pregunta: "¿Puedo tomarla con café?",
        respuesta: "Sí, de hecho la combinación L-teanina + cafeína mejora el enfoque sin nerviosismo."
      },
      {
        pregunta: "¿Causa dependencia?",
        respuesta: "No, la L-teanina no causa dependencia ni tolerancia."
      }
    ]
  },
  {
    nombre: "Potasio",
    descripcion: "Electrolito esencial para la función cardíaca, muscular y nerviosa. Contrarresta los efectos del sodio en la presión arterial.",
    beneficios: [
      "Regula la presión arterial",
      "Esencial para la función cardíaca normal",
      "Previene calambres musculares",
      "Apoya la función renal",
      "Reduce el riesgo de cálculos renales",
      "Importante para el equilibrio de fluidos"
    ],
    dosificacion: {
      general: "99 mg por dosis (límite legal en suplementos)",
      hombres: "3500-4700 mg diarios de fuentes alimentarias",
      mujeres: "2600-3500 mg diarios de fuentes alimentarias"
    },
    momento_optimo: "Con las comidas, distribuido durante el día",
    consejos_absorcion: "Mejor obtenerlo de alimentos. Suplementación solo bajo supervisión médica.",
    efectos_secundarios: [
      "Molestias gastrointestinales con suplementos",
      "Peligroso en exceso (hipercalemia)",
      "Contraindicado con ciertos medicamentos cardíacos"
    ],
    evidencia_cientifica: [
      "Reduce presión arterial sistólica en 3-5 mmHg",
      "Disminuye riesgo de ACV en 24% con ingesta adecuada",
      "Reduce formación de cálculos renales en 35-50%"
    ],
    productosMercadoLibre: [
      {
        marca: "Now Foods Potassium Citrate",
        url: "https://www.mercadolibre.com.uy/citrato-potasio",
        precio: 890,
        disponible: true
      }
    ],
    preguntas_frecuentes: [
      {
        pregunta: "¿Por qué los suplementos de potasio tienen dosis tan bajas?",
        respuesta: "Por regulación, los suplementos están limitados a 99mg. Es mejor obtenerlo de alimentos."
      },
      {
        pregunta: "¿Qué alimentos son ricos en potasio?",
        respuesta: "Plátanos, espinacas, aguacates, papas, frijoles y yogur son excelentes fuentes."
      }
    ]
  },
  {
    nombre: "Cromo",
    descripcion: "Mineral traza esencial que mejora la acción de la insulina y el metabolismo de carbohidratos, proteínas y grasas.",
    beneficios: [
      "Mejora la sensibilidad a la insulina",
      "Ayuda a regular los niveles de glucosa en sangre",
      "Puede reducir los antojos de carbohidratos",
      "Apoya el metabolismo de macronutrientes",
      "Puede ayudar en la pérdida de peso moderada",
      "Mejora el perfil lipídico en algunos casos"
    ],
    dosificacion: {
      general: "200-400 mcg diarios",
      hombres: "200-400 mcg con las comidas",
      mujeres: "200-400 mcg con las comidas"
    },
    momento_optimo: "Con las comidas principales, especialmente las que contienen carbohidratos",
    consejos_absorcion: "Mejor absorción con vitamina C. Evitar con antiácidos. Separar de hierro y zinc.",
    efectos_secundarios: [
      "Generalmente seguro en dosis recomendadas",
      "Puede causar irritación estomacal en algunos casos",
      "Mareos o dolor de cabeza ocasionales",
      "Interacciones con medicamentos para diabetes"
    ],
    evidencia_cientifica: [
      "Reduce glucosa en ayunas en 15-20 mg/dL en diabéticos tipo 2",
      "Mejora sensibilidad a insulina en 8-16% en estudios controlados",
      "Puede reducir peso corporal en 1-2 kg en 12 semanas con dieta"
    ],
    productosMercadoLibre: [
      {
        marca: "Now Foods Chromium Picolinate",
        url: "https://www.mercadolibre.com.uy/picolinato-cromo",
        precio: 750,
        disponible: true
      }
    ],
  ...
  },
  {
    ...
    preguntas_frecuentes: [
      {
        pregunta: "¿El picolinato de cromo es la mejor forma?",
        respuesta: "Sí, el picolinato de cromo tiene la mejor absorción y más estudios que lo respaldan."
      },
      {
        pregunta: "¿Ayuda realmente con la pérdida de peso?",
        respuesta: "Los efectos son modestos (1-2 kg) y funcionan mejor combinado con dieta y ejercicio."
      }
    ],
    productosMercadoLibre: [
      {
